...
const getStationID = (s) => {
	const body = {
		'results[5][4][function]': 'ws_info_stop',
		'results[5][4][data]': JSON.stringify([
			{ name: 'results[5][4][stop]', value: s },
			{ name: 'results[5][4][date]', value: moment.tz('Europe/Berlin').add(1, 'days').format('DD.MM.YYYY') },
			{ name: 'results[5][4][time]', value: '' },
			{ name: 'results[5][4][mode]', value: 'stop' }
		])
	}

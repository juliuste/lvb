...
const journeys = (origin, destination, date = Date.now(), opt = {}) => {
	...
	const body = {
		'results[5][4][function]': 'ws_find_connections',
		'results[5][4][data]': JSON.stringify([
			{ name: 'results[5][4][is_extended]', value: '1' },
			{ name: 'results[5][4][from]', value: origin },
			{ name: 'results[5][4][to]', value: destination },
			{ name: 'results[5][4][via]', value: options.via || '' },
			{ name: 'results[5][4][time_mode]', value: 'departure' },
			{ name: 'results[5][4][time]', value: time },
			{ name: 'results[5][4][date]', value: day },
			{ name: 'results[5][4][means_of_transport][]', value: 'STR' },
			{ name: 'results[5][4][means_of_transport][]', value: 'BUS' },
			{ name: 'results[5][4][means_of_transport][]', value: 'S/U' },
			{ name: 'results[5][4][means_of_transport][]', value: 'RB/RE' },
			{ name: 'results[5][4][mode]', value: 'connection' }
		])
	}

	return fetch('https://www.l.de/verkehrsbetriebe/fahrplan/verbindung', {
		method: 'post',
		body: stringify(body),
		headers: {
